...
def setup_logger():
    if "logger" not in st.session_state:
        ...
        if not logger.hasHandlers():
            ...
            file_handler = logging.FileHandler(f"{home_dir}/log/app.log")
...
def unique_preprocess_text(text):
    with open(f'{home_dir}/data/AItarentee/removewords.json', 'r', encoding='utf-8') as file:
        data = json.load(file)
    ...
    if st.sidebar.button("データ整形"):
        if st.session_state.df.empty:
            ...
            try:
                ...
                df_cleaned.to_csv(f"{home_dir}/data/df_cleaned.csv", index=False)
...
# =========================================================
    ...
    # すでに整形済みデータを読み込む
    try:
        dfg = pd.read_csv(f"{home_dir}/data/df_cleaned.csv")
    except:
        dfg = pd.DataFrame()

    if dfg.empty:
        st.error("整形済みデータ (df_cleaned.csv) が存在しません。")
